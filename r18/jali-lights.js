	...
	JALI.addLightsBox = function() {
		...
		JALI.lightsBox.innerHTML =
			'<p>' +
				...
				'<input type=color id=colLightAmbient value=#888888 > <output id=outLightAmbient >#888888</output><br>' +
			'</p>' +
			'<p>' +
				'<input type=checkbox id=chkLightCamera onclick=JALI.toggleLightCamera(); checked /> Light at camera position<br>' +
				'<input type=color id=colLightCamera value=#ffffff /> <output id=outLightCamera >#ffffff</output> ' +
			...
			'<p>' +
				...
				'<input type=color id=colLightPosition value=#ffffff /> <output id=outLightPosition >#ffffff</output> ' +
	...
	JALI.toggleLightAmbient = function() {
		if ( chkLightAmbient.checked === true ) {
			JALI.lightAmbient = new THREE.AmbientLight( colLightAmbient.value );
	...
	JALI.toggleLightCamera = function() {
		if ( chkLightCamera.checked === true ) {
			JALI.lightCamera = new THREE.PointLight( colLightCamera.value, 0.5 );
			JALI.lightCamera.position = JATH.camera.position;

			JATH.camera.add( JALI.lightCamera );
			JATH.scene.add( JATH.camera );
			JALI.updateMaterials( JATH.scene.children );
...
*/
	...
	JALI.toggleLightPosition = function( d ) {
		if ( chkLightPosition.checked === true ) {
			JALI.lightPosition = new THREE.DirectionalLight( 0x555555, 0.5 );  // 0xffffff 1.0
